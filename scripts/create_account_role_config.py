    ...
    for entry in sorted(os.listdir(accounts_dir)):
        ...
        if os.path.isdir(path) and any(f.endswith(".tf") for f in os.listdir(path)):
            subprocess.check_call(
                ["terraform", "init"], cwd=path, stdout=subprocess.DEVNULL
            )
            outputs = json.loads(
            ...
            for op in outputs.values():
                if (
                    ...
                    and op["value"].startswith("arn:aws:iam:")
                    and "role/" in op["value"]
                ):
                    yield op["value"]
...
def create_cli_credentials(*, accounts_dir, roles):
    ...
    with open(credentials_path, "w") as outfile:
        outfile.write(
            "; This config is autogenerated by scripts/create_account_role_config.py\n"
        )
